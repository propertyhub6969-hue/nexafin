...
const importer = require('./importer'); // parser CSV/XLSX (dipakai ulang utk impor aset)

// Petakan header kolom (fleksibel) → field aset, untuk impor Excel/CSV.
function mapAssetHeader(h) {
  h = String(h || '').toLowerCase().trim();
  if (/akumulasi/.test(h)) return 'akunAkumulasi';
  if (/beban/.test(h)) return 'akunBeban';
  if (/akun/.test(h) && /aset/.test(h)) return 'akunAset';
  if (/residu/.test(h)) return 'nilaiResidu';
  if (/tanggal|tgl/.test(h)) return 'tanggalPerolehan';
  if (/harga|nilai perolehan/.test(h)) return 'harga';
  if (/masa|manfaat|umur/.test(h)) return 'masaManfaat';
  if (/metode/.test(h)) return 'metode';
  if (/kelompok|fiskal|golongan/.test(h)) return 'kelompokFiskal';
  if (/nama/.test(h)) return 'nama';
  return null;
}
...
async function handle(req, res, ctx) {
  ...
  if (rest === '/journals' && method === 'POST') {
    ...
    const status = (firmSide || user.autoApprove) ? 'disetujui' : 'draf';  // sisi firma / klien-staff berizin langsung → disetujui; selain itu draf
  ...
  // Impor massal aset dari Excel (.xlsx) / CSV. Body: {kind:'xlsx'|'csv', base64?/text?}.
  if (rest === '/assets/import' && method === 'POST') {
    if (!firmSide) { send(res, 403, { error: 'Hanya konsultan/staf firma yang dapat mengimpor aset.' }); return true; }
    const b = await readBody(req);
    let grid;
    try {
      if (b.kind === 'xlsx') grid = importer.readXlsx(Buffer.from(b.base64 || '', 'base64'));
      else grid = importer.parseCSV(b.text || '');
    } catch (e) { send(res, 400, { error: 'Gagal membaca berkas: ' + e.message }); return true; }
    grid = (grid || []).filter(row => Array.isArray(row) && row.some(c => String(c == null ? '' : c).trim() !== ''));
    if (grid.length < 2) { send(res, 400, { error: 'Berkas kosong atau hanya berisi header. Butuh minimal 1 baris data.' }); return true; }
    const header = grid[0].map(c => String(c == null ? '' : c));
    const col = {};
    header.forEach((h, i) => { const f = mapAssetHeader(h); if (f && col[f] === undefined) col[f] = i; });
    if (col.nama === undefined || col.harga === undefined || col.tanggalPerolehan === undefined) {
      send(res, 400, { error: 'Kolom wajib tidak ditemukan. Pastikan ada header: Nama, Tanggal Perolehan, dan Harga (unduh template untuk contoh).' }); return true;
    }
    const validAkun = new Set(acc.accountsOf(cid).map(x => x.code));
    const get = (row, f) => col[f] !== undefined ? String(row[col[f]] == null ? '' : row[col[f]]).trim() : '';
    const num = (v) => Math.round((importer.parseAmount(v).value) || 0);
    const normMetode = (v) => /saldo|menurun|declin|(^|\s)sm(\s|$)/i.test(v || '') ? 'saldo-menurun' : 'garis-lurus';
    const normFiskal = (v) => {
      v = String(v || '').trim(); if (assets.FISKAL[v]) return v;
      const u = v.toUpperCase();
      if (/BANGUNAN/.test(u) && /(NON|TIDAK|SEMENTARA)/.test(u)) return 'bangunan-non-permanen';
      if (/BANGUNAN/.test(u)) return 'bangunan-permanen';
      if (/TANAH|TIDAK.*SUSUT|NON.?PENYUSUTAN/.test(u)) return 'non-penyusutan';
      const rom = (u.match(/\b(IV|III|II|I)\b/) || [])[1]; if (rom && assets.FISKAL[rom]) return rom;
      const ang = { '1': 'I', '2': 'II', '3': 'III', '4': 'IV' }[u.replace(/[^\d]/g, '')]; if (ang) return ang;
      return 'non-penyusutan';
    };
    let dibuat = 0; const errors = []; const created = [];
    for (let i = 1; i < grid.length; i++) {
      const row = grid[i], baris = i + 1;
      const nama = get(row, 'nama');
      const tgl = importer.parseDate(get(row, 'tanggalPerolehan'));
      const harga = num(get(row, 'harga'));
      if (!nama) { errors.push(`Baris ${baris}: nama kosong — dilewati.`); continue; }
      if (!/^\d{4}-\d{2}-\d{2}$/.test(tgl || '')) { errors.push(`Baris ${baris} (${nama}): tanggal perolehan tidak valid — dilewati.`); continue; }
      if (!(harga > 0)) { errors.push(`Baris ${baris} (${nama}): harga tidak valid — dilewati.`); continue; }
      const aAset = get(row, 'akunAset'), aAkum = get(row, 'akunAkumulasi'), aBeban = get(row, 'akunBeban');
      created.push({
        id: store.id(), companyId: cid, nama, tanggalPerolehan: tgl, harga,
        nilaiResidu: num(get(row, 'nilaiResidu')), metode: normMetode(get(row, 'metode')),
        masaManfaat: num(get(row, 'masaManfaat')), kelompokFiskal: normFiskal(get(row, 'kelompokFiskal')),
        metodeFiskal: 'garis-lurus',
        akunAset: validAkun.has(aAset) ? aAset : '', akunAkumulasi: validAkun.has(aAkum) ? aAkum : '', akunBeban: validAkun.has(aBeban) ? aBeban : '',
        catatan: '', aktif: true, penyusutanPosted: [], createdAt: new Date().toISOString()
      });
      dibuat++;
    }
    if (created.length) { store.db().assets = store.db().assets || []; created.forEach(a => store.db().assets.push(a)); store.saveNow(); }
    send(res, 200, { dibuat, gagal: errors.length, errors: errors.slice(0, 60) });
    return true;
  }
  ...
  // Pelepasan/penjualan aset: posting penyusutan s/d tgl lepas → jurnal pelepasan (laba/rugi) → stop penyusutan.
  if (/^\/assets\/[^/]+\/dispose$/.test(rest) && method === 'POST') {
    if (!firmSide) { send(res, 403, { error: 'Hanya konsultan/staf firma yang dapat melepas aset.' }); return true; }
    const a = store.db().assets.find(x => x.id === seg(rest, 2) && x.companyId === cid);
    if (!a) { send(res, 404, { error: 'Aset tidak ditemukan.' }); return true; }
    if (a.dilepas) { send(res, 400, { error: 'Aset ini sudah dilepas sebelumnya.' }); return true; }
    const b = await readBody(req);
    const tanggal = String(b.tanggal || '');
    if (!/^\d{4}-\d{2}-\d{2}$/.test(tanggal)) { send(res, 400, { error: 'Tanggal pelepasan (YYYY-MM-DD) wajib diisi.' }); return true; }
    if (books.isLocked(cid, tanggal)) { send(res, 423, { error: `Periode ${books.periodeOf(tanggal)} terkunci — buka kunci dulu.` }); return true; }
    const hargaJual = Math.max(0, Math.round(Number(b.hargaJual) || 0));
    const valid = new Set(acc.accountsOf(cid).map(x => x.code));
    if (!a.akunAset || !a.akunAkumulasi) { send(res, 400, { error: 'Lengkapi dulu Akun Aset & Akun Akumulasi di menu Ubah aset.' }); return true; }
    if (!valid.has(a.akunAset) || !valid.has(a.akunAkumulasi)) { send(res, 400, { error: 'Akun aset/akumulasi tidak dikenal di buku ini.' }); return true; }
    if (hargaJual > 0 && (!b.akunKas || !valid.has(b.akunKas))) { send(res, 400, { error: 'Pilih akun kas/bank penerima hasil penjualan.' }); return true; }
    const sampai = assets.ymOf(tanggal);
    // 1) Posting penyusutan yang jatuh tempo s/d bulan pelepasan (agar akumulasi mutakhir).
    let penyusutanDiposting = 0;
    if (a.akunBeban && a.akunAkumulasi) {
      for (const m of assets.dueMonths(a, sampai)) {
        if (books.isLocked(cid, m.periode + '-01')) continue;
        const nn = 'JU-' + String(store.nextNumber(cid, 'journal')).padStart(5, '0');
        store.db().journals.push({ id: store.id(), companyId: cid,
          date: m.periode + '-' + String(new Date(Number(m.periode.slice(0, 4)), Number(m.periode.slice(5, 7)), 0).getDate()).padStart(2, '0'),
          number: nn, description: `Penyusutan ${a.nama} — ${m.periode}`, createdAt: new Date().toISOString(),
          status: 'disetujui', createdBy: user.id, createdByName: user.name, dariPenyusutan: a.id,
          lines: [{ accountCode: a.akunBeban, debit: m.amount, credit: 0 }, { accountCode: a.akunAkumulasi, debit: 0, credit: m.amount }] });
        a.penyusutanPosted = a.penyusutanPosted || []; a.penyusutanPosted.push(m.periode);
        penyusutanDiposting++;
    ...
    // 2) Akumulasi NYATA dari buku besar (jumlah kredit ke akunAkumulasi dari jurnal penyusutan aset ini)
    //    → jurnal pelepasan pasti seimbang & cocok GL (hindari selisih pembulatan).
    const akum = store.db().journals
      .filter(jj => jj.companyId === cid && jj.dariPenyusutan === a.id)
      .reduce((s, jj) => s + (jj.lines || []).reduce((ss, l) => ss + (l.accountCode === a.akunAkumulasi ? (l.credit || 0) : 0), 0), 0);
    const harga = Math.round(Number(a.harga) || 0);
    const nilaiBuku = harga - akum;
    const labaRugi = hargaJual - nilaiBuku;   // + = laba, − = rugi
    if (labaRugi !== 0 && (!b.akunLabaRugi || !valid.has(b.akunLabaRugi))) { send(res, 400, { error: 'Pilih akun Laba/Rugi Pelepasan Aset.' }); return true; }
    // 3) Jurnal pelepasan (seimbang secara aljabar): Dr Kas+Akum = Cr Aset + Laba (atau Dr Rugi).
    const lines = [];
    if (hargaJual > 0) lines.push({ accountCode: b.akunKas, debit: hargaJual, credit: 0 });
    if (akum > 0) lines.push({ accountCode: a.akunAkumulasi, debit: akum, credit: 0 });
    lines.push({ accountCode: a.akunAset, debit: 0, credit: harga });
    if (labaRugi > 0) lines.push({ accountCode: b.akunLabaRugi, debit: 0, credit: labaRugi });
    else if (labaRugi < 0) lines.push({ accountCode: b.akunLabaRugi, debit: -labaRugi, credit: 0 });
    const num = 'JU-' + String(store.nextNumber(cid, 'journal')).padStart(5, '0');
    const j = { id: store.id(), companyId: cid, date: tanggal, number: num,
      description: `Pelepasan aset: ${a.nama}${hargaJual > 0 ? ` (dijual ${hargaJual})` : ' (dibuang/scrap)'}`,
      createdAt: new Date().toISOString(), status: 'disetujui', createdBy: user.id, createdByName: user.name,
      dariPelepasan: a.id, lines };
    store.db().journals.push(j);
    // 4) Tandai aset dilepas + stop penyusutan (aktif=false → dilewati saat depreciate).
    a.aktif = false;
    a.dilepas = { tanggal, hargaJual, akum, nilaiBuku, labaRugi, jurnalId: j.id, jurnalNumber: num, at: new Date().toISOString(), by: user.id };
    store.saveNow();
    send(res, 200, { ok: true, nilaiBuku, akum, labaRugi, jurnalNumber: num, penyusutanDiposting });
    return true;
  }
