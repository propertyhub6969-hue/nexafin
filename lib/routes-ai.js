...
async function handle(req, res, ctx) {
  ...
  if (pathname === '/api/import/parse' && method === 'POST') {
    ...
    const scope = bookScope(user, query, b);
    if (books && !books.canWriteBook(user, scope)) { send(res, 403, { error: 'Mode baca-saja: hanya staf pembukuan/pengawas penanggung jawab yang dapat mengimpor ke buku klien ini.' }); return true; }
  ...
  if (/^\/api\/import\/[^/]+\/row$/.test(pathname) && method === 'POST') {
    ...
    if (!books.canWriteBook(user, batch.companyId)) { send(res, 403, { error: 'Mode baca-saja: tidak dapat mengubah baris impor buku klien ini.' }); return true; }
  ...
  if (/^\/api\/import\/[^/]+\/post$/.test(pathname) && method === 'POST') {
    ...
    const batch = d.imports.find(x => x.id === id);
    if (!batch || !books || !books.resolve(user, batch.companyId).ok) { send(res, 404, { error: 'Batch tidak ditemukan.' }); return true; }
    if (!books.canWriteBook(user, batch.companyId)) { send(res, 403, { error: 'Mode baca-saja: hanya staf pembukuan/pengawas penanggung jawab yang dapat memposting impor ke buku klien ini.' }); return true; }
    const scope = batch.companyId;
    const bankCode = batch.bankAccountCode;
    const valid = new Set(acc.accountsOf(scope).map(a => a.code));
    const impStatus = (books.isFirmSide(user) || user.autoApprove) ? 'disetujui' : 'draf';   // staf klien berizin → langsung; selain itu draf
  ...
  if (/^\/api\/import\/[^/]+$/.test(pathname) && method === 'DELETE') {
    const id = pathname.split('/')[3];
    const batch = d.imports.find(x => x.id === id);
    if (batch && books && books.resolve(user, batch.companyId).ok && books.canWriteBook(user, batch.companyId)) d.imports = d.imports.filter(x => x.id !== id);
    store.saveNow();
  ...
  if (pathname === '/api/rules' && method === 'POST') {
    ...
    const scope = bookScope(user, query, b);
    if (books && !books.canWriteBook(user, scope)) { send(res, 403, { error: 'Mode baca-saja: tidak dapat menambah aturan pada buku klien ini.' }); return true; }
  ...
  if (/^\/api\/rules\/[^/]+$/.test(pathname) && method === 'DELETE') {
    ...
    if (rule && books && books.resolve(user, rule.companyId).ok && books.canWriteBook(user, rule.companyId)) d.rules = d.rules.filter(r => r.id !== id);
    store.saveNow();
    send(res, 200, { ok: true });
    return true;
  }
  ...
  if (/^\/api\/import\/[^/]+\/bulk$/.test(pathname) && method === 'POST') {
    ...
    const batch = d.imports.find(x => x.id === id);
    if (!batch || !books || !books.resolve(user, batch.companyId).ok) { send(res, 404, { error: 'Batch tidak ditemukan.' }); return true; }
    if (!books.canWriteBook(user, batch.companyId)) { send(res, 403, { error: 'Mode baca-saja: tidak dapat menerapkan massal pada buku klien ini.' }); return true; }
    const scope = batch.companyId;
  ...
  /* ---------- Draf Surat DJP (grounded ke data klien) ---------- */
  if (pathname === '/api/ai/surat-djp' && method === 'POST') {
    if (books && !books.isFirmSide(user)) { send(res, 403, { error: 'Fitur ini untuk konsultan/staf firma.' }); return true; }
    const b = await readBody(req);
    const s = aiSettings();
    const cid = bookScope(user, query, b);
    const d = store.db();
    const client = d.clients.find(c => c.id === cid && c.companyId === user.companyId) || null;
    const firma = d.companies.find(c => c.id === user.companyId) || null;
    const dataKlien = {
      firma: firma ? firma.name : '',
      klien: client ? { nama: client.nama, npwp: client.npwp || '', jenisUsaha: client.jenisUsaha || '' } : null,
      keuangan: buildSummary(cid, b.from, b.to)
    };
    try {
      const text = await ai.draftSuratDJP({ key: s.aiKey, model: s.aiModel, jenis: b.jenis, konteks: b.konteks, dataKlien });
      send(res, 200, { text });
    } catch (e) { send(res, 400, { error: e.message }); }
    return true;
  }
