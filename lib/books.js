...
function visibleClientIds(user) {
  ...
  d.clients.forEach(c => {
    ...
    else if (pembukuanIds(c).some(id => scope.has(id))) ids.add(c.id);   // staf pembukuan (delegasi pengawas)
  });
  d.tasks.forEach(t => { if (t.companyId === user.companyId && scope.has(t.assignedTo)) ids.add(t.clientId); }); // perpajakan (SPT)
  return ids;
}
