...
function canWriteBook(user, clientId) {
  const { isAdmin, isPengawas, isStaff, isKlienStaff } = roleFlags(user);
  if (isAdmin) return true;
  if (isKlienStaff) return user.clientId === clientId;
  const d = store.db();
  const c = d.clients.find(x => x.id === clientId && x.companyId === user.companyId);
  if (!c) return false;
  if (isPengawas) return [user.id, ...teamIds(user)].includes(c.assignedTo);
  // staf: ditunjuk sebagai pembukuan, atau PIC/penanggung jawab langsung (kompat data lama)
  if (isStaff) return pembukuanIds(c).includes(user.id) || c.assignedTo === user.id;
  return false;
}
