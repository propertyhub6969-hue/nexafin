...
// Staf pembukuan yang ditunjuk pengawas untuk sebuah klien (Tingkat 2 — pembukuan).
function pembukuanIds(client) { return Array.isArray(client && client.pembukuanBy) ? client.pembukuanBy : []; }
function perpajakanIds(client) { return Array.isArray(client && client.perpajakanBy) ? client.perpajakanBy : []; }

// Himpunan clientId yang boleh DILIHAT bukunya oleh user (null = semua klien firma).
// Delegasi bertingkat: pengawas penanggung jawab (client.assignedTo), staf pembukuan
// (client.pembukuanBy), atau staf perpajakan (punya tugas SPT klien) — semuanya boleh
// MELIHAT buku (berbagi visibilitas). Hak MENULIS dipisah di canWriteBook().
function visibleClientIds(user) {
  ...
  d.clients.forEach(c => {
    ...
    else if (pembukuanIds(c).some(id => scope.has(id))) ids.add(c.id);    // pelaksana pembukuan
    else if (perpajakanIds(c).some(id => scope.has(id))) ids.add(c.id);   // pelaksana pajak (ditunjuk)
  });
  d.tasks.forEach(t => { if (t.companyId === user.companyId && scope.has(t.assignedTo)) ids.add(t.clientId); }); // pelaksana SPT (via tugas)
  return ids;
}

// Boleh MENULIS buku klien (jurnal/impor/aset/COA). Lebih ketat dari sekadar melihat:
// hanya admin, pengawas penanggung jawab, staf pembukuan yang ditunjuk, atau klien-staf
// (draf bukunya sendiri). Staf perpajakan yang hanya punya tugas SPT = baca-saja atas buku.
function canWriteBook(user, clientId) {
  const { isAdmin, isPengawas, isKlienStaff } = roleFlags(user);
  if (isAdmin) return true;
  if (isKlienStaff) return user.clientId === clientId;
  const d = store.db();
  const c = d.clients.find(x => x.id === clientId && x.companyId === user.companyId);
  if (!c) return false;
  // Role-agnostik: penanggung jawab (PJ) atau pelaksana pembukuan boleh menulis buku.
  if (c.assignedTo === user.id) return true;
  if (pembukuanIds(c).includes(user.id)) return true;
  // Kompat data lama: pengawas atas klien yang di-PIC-kan ke anggota timnya.
  if (isPengawas && teamIds(user).includes(c.assignedTo)) return true;
  return false;
}
...
module.exports = {
  roleFlags, isFirmSide, pembukuanIds, canWriteBook, visibleClientIds, canSeeClient, listBooks,
