...
// Izin per-orang (diberi admin di Tim/Staff): akses penjurnalan SEMUA buku klien firma.
function hasBukuKlienPerm(user) {
  return !!(user && user.perms && user.perms.jurnalKlien);
}
...
function visibleClientIds(user) {
  ...
  // Izin 'jurnalKlien': staf firma/pengawas berizin dapat membuka SEMUA buku klien firma
  // (null = semua klien; buku firma tetap khusus admin, dijaga terpisah di listBooks/resolve).
  if (hasBukuKlienPerm(user)) return null;
  const d = store.db();
...
module.exports = {
  roleFlags, isFirmSide, hasBukuKlienPerm, visibleClientIds, canSeeClient, listBooks,
