...
**Catatan Pajak** — 2-3 poin dari sudut perpajakan Indonesia (mis. potensi koreksi fiskal, kesesuaian PPN Masukan vs pembelian, beban yang mungkin non-deductible, kewajiban PPh yang timbul). Jangan mengarang angka di luar data.
...
${JSON.stringify(ringkasan)}`;
  const { text } = await requestClaude({
    ...
    system: 'Anda analis keuangan & pajak berpengalaman, ringkas, jujur, dan praktis. Jangan mengarang angka di luar data.',
    messages: [{ role: 'user', content: [{ type: 'text', text: instruksi }] }]
  });
  return text;
}

/* Draf surat/tanggapan DJP (grounded ke data klien) */
async function draftSuratDJP({ key, model, jenis, konteks, dataKlien }) {
  const jenisMap = {
    'sp2dk': 'Surat Tanggapan atas SP2DK (Surat Permintaan Penjelasan atas Data dan/atau Keterangan)',
    'keberatan': 'Surat Keberatan',
    'pengurangan-sanksi': 'Surat Permohonan Pengurangan/Penghapusan Sanksi Administrasi',
    'klarifikasi': 'Surat Klarifikasi kepada DJP'
  };
  const judul = jenisMap[jenis] || 'Surat kepada DJP';
  const instruksi =
`Anda konsultan pajak Indonesia berpengalaman. Susun DRAF ${judul} yang profesional, sopan, dan sesuai kaidah surat resmi perpajakan Indonesia.

KONTEKS/PERMINTAAN dari pengguna (isi surat DJP / poin yang harus ditanggapi):
${konteks || '(tidak ada — buat kerangka umum yang bisa dilengkapi konsultan)'}

DATA KLIEN & KEUANGAN (untuk mendukung argumen; pakai hanya yang relevan, JANGAN mengarang angka di luar ini):
${JSON.stringify(dataKlien)}

Aturan:
- Bahasa Indonesia formal, struktur surat resmi (identitas pengirim/kuasa, nomor & tanggal, perihal, "Kepada Yth. Kepala KPP ...", pembuka, isi/argumen, penutup, tanda tangan). Isi bagian yang tak ada di data dengan placeholder dalam kurung siku, mis. [Nomor Surat], [Tanggal], [Nama KPP].
- Untuk SP2DK: tanggapi tiap poin, jelaskan berdasarkan data, sebut dasar hukum yang relevan (pasal UU KUP/PPh/PPN/PMK) bila sesuai.
- Beri penanda [PERIKSA] pada bagian yang WAJIB dicek konsultan (angka, tanggal, nomor, nama).
- Ini DRAF — akhiri dengan catatan: "Draf ini perlu diperiksa & disesuaikan konsultan sebelum dikirim."
- Keluarkan HANYA teks suratnya (boleh Markdown ringan).`;
  const { text } = await requestClaude({
    key, model, max_tokens: 2600,
    system: 'Anda konsultan pajak Indonesia yang teliti, patuh aturan, dan TIDAK mengarang fakta/angka/pasal di luar data yang diberikan. Bila ragu, tandai untuk diperiksa konsultan.',
    messages: [{ role: 'user', content: [{ type: 'text', text: instruksi }] }]
  });
  return text;
}

module.exports = { requestClaude, extractDocument, generateInsight, draftSuratDJP, extractJSON };
