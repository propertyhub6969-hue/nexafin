...
async function handle(req, res, ctx) {
  ...
  if (pathname === '/api/consult/meta' && method === 'GET') {
    ...
    send(res, 200, { kategoriDokumen: C.KATEGORI_DOKUMEN, jenisSPT: C.JENIS_SPT, statusTugas: C.STATUS_TUGAS, statusInvoice: C.STATUS_INVOICE, jenisUsaha: C.JENIS_USAHA,
      role: user.role, perms: user.perms || {}, canInvoice: canInv, isAdmin, isPengawas, isPJ,
  ...
  if (pathname === '/api/staff' && method === 'POST') {
    ...
    // Admin bebas (staff/pengawas/klien-staff); PJ hanya boleh buat staff firma atau klien-staff.
    const role = isAdmin
      ? (['pengawas', 'klien-staff'].includes(b.role) ? b.role : 'staff')
      : (b.role === 'klien-staff' ? 'klien-staff' : 'staff');
