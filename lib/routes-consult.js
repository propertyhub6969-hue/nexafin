...
async function handle(req, res, ctx) {
  ...
  const myPJClientIds = new Set(d.clients.filter(c => c.companyId === cid && c.assignedTo === user.id).map(c => c.id));
  const isPJ = myPJClientIds.size > 0;
  ...
  const canManageAccount = (u) => {
    if (isAdmin) return true;
    if (!isPJ) return false;
    if (u.role === 'admin' || u.role === 'user') return false;
    if (u.role === 'klien-staff') return myPJClientIds.has(u.clientId);   // PJ klien tsb boleh kelola staf kliennya
    return myRosterUserIds().has(u.id);
  };
  ...
  if (pathname === '/api/staff' && method === 'GET') {
    ...
    const list = members.map(u => ({ id: u.id, name: u.name, email: u.email, role: u.role, perms: u.perms || {},
      ...
      clientId: u.clientId || null, clientName: u.clientId ? (clientNameById[u.clientId] || '') : '', autoApprove: !!u.autoApprove }));
  ...
  if (pathname === '/api/staff' && method === 'POST') {
    ...
    const u = { id: store.id(), name, email, salt, passwordHash: hash, role, companyId: cid, perms: {},
      supervisorId: (isAdmin && role === 'staff') ? (b.supervisorId || null) : null, clientId,
      autoApprove: role === 'klien-staff' ? !!b.autoApprove : false, createdAt: new Date().toISOString() };
  ...
  if (/^\/api\/staff\/[^/]+$/.test(pathname) && method === 'PUT') {
    ...
    // Toggle: jurnal staf klien langsung disetujui (tanpa persetujuan konsultan)
    if (b.autoApprove !== undefined && u.role === 'klien-staff') u.autoApprove = !!b.autoApprove;
    store.saveNow();
