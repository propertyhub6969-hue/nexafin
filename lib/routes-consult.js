...
async function handle(req, res, ctx) {
  ...
  const bukuKlienAll = isAdmin || !!(user.perms && user.perms.jurnalKlien); // akses penjurnalan SEMUA buku klien
  ...
  const visibleClientIds = () => {
    ...
    const ids = new Set();
    d.clients.forEach(c => { if (c.companyId === cid && scope.has(c.assignedTo)) ids.add(c.id); });
    d.tasks.forEach(t => { if (t.companyId === cid && scope.has(t.assignedTo)) ids.add(t.clientId); });
    return ids;
  };
  ...
  if (/^\/api\/staff\/[^/]+\/perms$/.test(pathname) && method === 'POST') {
    ...
    if (b.jurnalKlien !== undefined) u.perms.jurnalKlien = !!b.jurnalKlien;
    store.saveNow();
  ...
  if (pathname === '/api/tasks' && method === 'GET') {
    ...
    // pelingkupan peran: staff hanya tugasnya, pengawas hanya timnya, admin semua
    if (!isAdmin) { const scope = new Set(myScope()); list = list.filter(t => scope.has(t.assignedTo)); }
    ...
    list = list.map(t => { const note = C.deadlineNote(t.jenis, t.deadline); return Object.assign({}, t, {
        ...
        punyaBukti: !!(t.bukti || t.buktiLink) }); })
  ...
  if (/^\/api\/documents\/[^/]+\/file$/.test(pathname) && method === 'GET') {
    ...
    // Pemegang izin 'jurnalKlien' boleh membuka lampiran (bukti) buku klien mana pun.
    if (!canSeeClient(doc.clientId) && !bukuKlienAll) { send(res, 403, { error: 'Anda tidak berwenang membuka dokumen ini.' }); return true; }
    try {
      const fs = require('fs');
