...
async function handle(req, res, ctx) {
  ...
  const kelolaTugas = isAdmin || !!(user.perms && user.perms.kelolaTugas); // boleh ubah tugas orang lain
  ...
  const canEditTask = (t) => { if (isAdmin || kelolaTugas) return true; if (isPengawas) return [user.id, ...teamIds()].includes(t.assignedTo); return t.assignedTo === user.id; };
  ...
  const visibleClientIds = () => {
    ...
    d.clients.forEach(c => {
      ...
      if (scope.has(c.assignedTo)) ids.add(c.id);                                                   // pengawas penanggung jawab / PIC
      else if (Array.isArray(c.pembukuanBy) && c.pembukuanBy.some(x => scope.has(x))) ids.add(c.id); // staf pembukuan (delegasi)
    });
    d.tasks.forEach(t => { if (t.companyId === cid && scope.has(t.assignedTo)) ids.add(t.clientId); }); // perpajakan (SPT)
    return ids;
  };
  // Sanitasi daftar staf pembukuan: hanya peran staff dalam firma; pengawas dibatasi timnya.
  const sanitizePembukuan = (arr) => {
    if (!Array.isArray(arr)) return undefined;
    const validStaff = new Set(d.users.filter(u => u.companyId === cid && u.role === 'staff').map(u => u.id));
    let ids = arr.filter(x => validStaff.has(x));
    if (isPengawas && !isAdmin) { const team = new Set(teamIds()); ids = ids.filter(x => team.has(x)); }
    return [...new Set(ids)];
  };
  ...
  if (pathname === '/api/consult/meta' && method === 'GET') {
    ...
    send(res, 200, { kategoriDokumen: C.KATEGORI_DOKUMEN, jenisSPT: C.JENIS_SPT, statusTugas: C.STATUS_TUGAS, statusInvoice: C.STATUS_INVOICE, jenisUsaha: C.JENIS_USAHA,
      role: user.role, perms: user.perms || {}, canInvoice: canInv, isAdmin, isPengawas, kelolaTugas,
  ...
  if (pathname === '/api/staff' && method === 'GET') {
    ...
    if (isPengawas) { const tset = new Set([user.id, ...teamIds()]); members = members.filter(u => tset.has(u.id)); }
    else if (isStaff) { members = members.filter(u => u.id === user.id); }
  ...
  if (pathname === '/api/staff' && method === 'POST') {
    ...
    const role = ['pengawas', 'klien-staff'].includes(b.role) ? b.role : 'staff';
    // klien-staff wajib terikat ke satu klien milik firma
    let clientId = null;
    if (role === 'klien-staff') {
    ...
    const u = { id: store.id(), name, email, salt, passwordHash: hash, role, companyId: cid, perms: {},
      supervisorId: role === 'staff' ? (b.supervisorId || null) : null, clientId, createdAt: new Date().toISOString() };
  ...
  /* Ubah peran / pengawas anggota tim */
  if (/^\/api\/staff\/[^/]+$/.test(pathname) && method === 'PUT') {
    ...
    const b = await readBody(req);
    if ((b.role === 'staff' || b.role === 'pengawas') && u.id !== user.id && u.role !== 'admin') {
      u.role = b.role; if (b.role === 'pengawas') u.supervisorId = null;
    }
    if (b.supervisorId !== undefined && u.role === 'staff') u.supervisorId = b.supervisorId || null;
    store.saveNow();
  ...
  if (/^\/api\/staff\/[^/]+\/perms$/.test(pathname) && method === 'POST') {
    ...
    if (b.kelolaTugas !== undefined) u.perms.kelolaTugas = !!b.kelolaTugas;
    store.saveNow();
  ...
  if (pathname === '/api/clients' && method === 'POST') {
    ...
    const c = { id: store.id(), companyId: cid, nama: b.nama.trim(), npwp: b.npwp || '', jenisUsaha: b.jenisUsaha || '',
      ...
      assignedTo, pembukuanBy: sanitizePembukuan(b.pembukuanBy) || [], createdAt: new Date().toISOString() };
    d.clients.push(c);
  ...
  if (/^\/api\/clients\/[^/]+$/.test(pathname) && method === 'PUT') {
    ...
    const b = await readBody(req);
    ['nama', 'npwp', 'jenisUsaha', 'pic', 'email', 'telepon', 'status', 'assignedTo'].forEach(k => { if (b[k] !== undefined) c[k] = b[k]; });
    if (isPengawas && !isAdmin) { const allowed = [user.id, ...teamIds()]; if (c.assignedTo && !allowed.includes(c.assignedTo)) c.assignedTo = user.id; }
    // Delegasi pembukuan (Tingkat 2): pengawas menunjuk staf timnya; admin bebas staf mana pun.
  ...
  if (/^\/api\/tasks\/[^/]+$/.test(pathname) && method === 'DELETE') {
    ...
    const bolehHapus = isAdmin || kelolaTugas || (isPengawas && [user.id, ...teamIds()].includes(th.assignedTo));
